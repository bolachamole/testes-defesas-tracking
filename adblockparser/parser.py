...
class AdblockRules(object):
    ...
    def __init__(self, rules, supported_options=None, skip_unsupported_rules=True,
                 use_re2=None, max_mem=256*1024*1024, rule_cls=AdblockRule):
        ...
        self.uses_re2 = _is_re2_supported() if use_re2 == None else use_re2
...
def _combined_regex(regexes, flags=re.IGNORECASE, use_re2=False, max_mem=None):
    ...
    re2 library (https://github.com/google/re2) often can match and compile
    ...
    * in case of "DFA out of memory" errors use ``max_mem`` argument
    ...
    if use_re2:
        ...
        if flags & re.S:
            options.dot_nl = True
        return re2.compile(joined_regexes, options)
